	...
	**/
	...
	public AccountDto getUserAccount(int userSeq, int accountSeq) throws UserAccountNotFoundException {
		for (AccountDto aDto : accountList) {
			if (aDto.getUserSeq() == userSeq && aDto.getAccountSeq() == accountSeq) return aDto;
		}
		throw new UserAccountNotFoundException("계좌 없음");
	}

	@Override
	public int withdraw(int userSeq, int accountSeq, int amount) throws BalanceLackException, UserAccountNotFoundException {
		AccountDto aDto = this.getUserAccount(userSeq,accountSeq);
		int money = aDto.getBalance()-amount;
		if(money<0) throw new BalanceLackException("잔고 없음");
		aDto.setBalance(money);
