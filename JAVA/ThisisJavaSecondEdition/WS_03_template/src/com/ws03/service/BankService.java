	...
	*/
	public AccountDto[] getAccountList(int userSeq) {
		...
		for(int i=0;i<accountList.length;i++) {
			if(this.accountList[i] != null) 
				if(this.accountList[i].getUserSeq()==userSeq) {
					cnt++;
				}
		}

		AccountDto[] accounts = new AccountDto[cnt];
		for(int i=0;i<cnt;i++) {
			if(this.accountList[i] != null) {
				if(this.accountList[i].getUserSeq()==userSeq) {
					accounts[i] = this.accountList[i];
				}
			}
		}
		return accounts;
	...
	**/
	public UserDto getUserDetail(int userSeq) {
		for(int i=0;i<this.userList.length;i++) {
			if(this.userList[i].getUserSeq()==userSeq) return this.userList[i];
		}
